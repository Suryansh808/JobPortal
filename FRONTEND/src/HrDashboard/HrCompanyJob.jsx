...
const HrCompanyJob = () => {
  ...
  useEffect(() => {
    const hrName = localStorage.getItem('HrName'); // Retrieve HrName from local storage
    if (hrName) {
      // Filter jobs based on HrName
      setFilteredJobs(companyjobs.filter(job => job.hrName === hrName ));
    } else {
      setFilteredJobs(companyjobs);
    }
  }, [companyjobs]); // Trigger this effect when companyjobs change
