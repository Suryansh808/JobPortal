...
const List = () => {
  ...
  //fetching jobs filter by HR Name
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        const storedHrName = localStorage.getItem("HrName");

        // Filter jobs that match the stored company name
        const filteredJobs = allJobs.filter(
          (job) => job.hrName === storedHrName
        );
  ...
  const preStyle = {
    // minWidth: "20%", // Limit maximum width
    // width: "auto",
  ...
  //  get the application data of companyUpdate
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        ...
        console.log("application datas", response.data);
        setApplications(response.data);
        console.log("fetch application data", applications);
      } catch (error) {
  ...
  // end here companyUpdates datas
  //update the company update to hrUpdate in the database
  const currentDate = new Date();
    const formattedDate = `${currentDate.getDate().toString().padStart(2, '0')}-${(currentDate.getMonth() + 1).toString().padStart(2, '0')}-${currentDate.getFullYear()}`
  const handleUpdate = async (lastIndexValue, user, selectedJob) => {
    if (!lastIndexValue) {
      alert("No company to update.");
      return;
    }
    ...
    const dataToSend = {
      ...
      date: formattedDate,
      userId: user._id, // Pass the userId from the user object
      jobId: selectedJob._id, // Pass the jobId from the selected job object
    };

    try {
      // Make a request to your backend to push data to MongoDB
      await axios.post("http://localhost:5000/api/updateCompany", dataToSend);
        ...
        <div className="w-full p-2 h-[80vh] relative">
          {selectedJob ? (
            <div className="h-full w-full bg-white rounded-lg p-1 group flex gap-3">
              <div className="mb-2 w-full rounded-md shadow-sm px-2 py-2 overflow-y-auto scrollbar-hide">
                ...
                <div className="overflow-x-auto text-black mt-4">
                  ...
                  <div className="flex  items-center justify-end px-3 py-1">
                    <input
                      ...
                      placeholder="write anything for users.."
                    />
                    <AiFillEdit
                        ...
                        <tr key={index}>
                          ...
                          <td className="border border-gray-300 px-2 py-1">
                            {(() => {
                              ...
                              // Access the user ID directly if user doesn't have userId
                              ...
                              if (
                                ...
                                return (
                                  <ul>
                                    <li
                                      key={
                                        application.companyUpdated.length - 1
                                      }
                                    >
                                      {
                                        application.companyUpdated[
                                      ...
                                      application.hrUpdated.round[
                                        ...
                                        <button
                                          className="border px-2 py-1 rounded-lg text-sm ml-2 bg-blue-600"
                                          onClick={() =>
                                            handleUpdate(
                                              application.companyUpdated[
                                                application.companyUpdated
           ...
           {/* //chat box starts */}
              <div className="absolute">
                <button
                  className="bg-blue-600 fixed bottom-14 right-4 text-white px-2 py-2 rounded-xl shadow-lg hover:bg-blue-700 transition-all duration-300"
                  onClick={() => setIsChatOpen(!isChatOpen)} // Toggle chat box visibility
                >
                  {isChatOpen ? "Close Chat" : "Chat with Company"}
                </button>

                {isChatOpen && (
                  <div className="fixed bottom-10 right-4 w-[25vw] min-w-[350px] bg-white border border-gray-300 rounded-lg shadow-lg overflow-hidden">
                    <div className="bg-blue-600 text-white py-3 px-4 flex justify-between items-center">
                      <h1 className="text-lg font-semibold">
                        Chat with {selectedJob ? (selectedJob.companyName) : null}
                      </h1>
                      <button
                    ...
                    <div className="h-[50vh] bg-cover bg-[url('https://images.unsplash.com/photo-1478760329108-5c3ed9d495a0?q=80&w=1674&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D')] py-3 overflow-y-scroll">
                      {chatMessages.map((msg, index) => (
                        <div
                          key={index}
                          className={`flex ${
                            msg.user === "cm" ? "justify-start" : "justify-end"
                          } mb-3`}
                        >
                          <div
                          style={preStyle}
                    ...
                    <div className="flex items-center border-t border-gray-300 p-3 bg-gray-50">
                      ...
                      />
                      <button
                        className="ml-3 bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg hover:bg-blue-700 transition-all duration-300"
